...
export default function BonsaiShop() {
  ...
  // Get chat control from window (set by BonsaiShopAgent)
  useEffect(() => {
    const checkChatControl = () => {
      if ((window as any).bonsaiChatControl) {
        setChatControl((window as any).bonsaiChatControl);
      }
    };
    
    checkChatControl();
    const interval = setInterval(checkChatControl, 500);
    return () => clearInterval(interval);
  }, []);

  const askAIAboutProduct = async (product: Product) => {
    // Open the chat panel first
    if ((window as any).openBonsaiChat) {
    ...
    if (chatControl) {
      try {
        await chatControl.sendFlowRequest({ text_input: message });
      } catch (error) {
        console.error('Failed to send message to chat:', error);
      }
    } else {
      console.warn('Chat control not available yet');
    }
  };
  ...
  const handleCheckout = () => {
    ...
    router.push('/examples/chat-sdk/bonsai-shop/checkout');
  };
...
function ProductCard({ product, onAddToCart }: { product: Product; onAddToCart: (product: Product, event?: React.MouseEvent) => void }) {
  ...
  const handleCardClick = () => {
    router.push(`/examples/chat-sdk/bonsai-shop/product/${product.id}`);
  };
